...
Meteor.user = function () {
  ...
  if (!userId)
    return null;
  return Meteor.users.findOne({meteor_id: userId});
};
...
var attemptLogin = function (methodInvocation, methodName, methodArgs, result) {
  ...
  var user;
  console.log('Attempting to find ', result.userId);
  if (result.userId)
    user = Meteor.users.findOne({meteor_id: result.userId});
...
Accounts._reportLoginFailure = function (methodInvocation, methodName, methodArgs, result) {
  ...
  if (result.userId)
    attempt.user = Meteor.users.findOne({meteor_id: result.userId});
...
Accounts.destroyToken = function (userId, loginToken) {
  Meteor.users.update({meteor_id: userId}, {
    $pull: {
...
Meteor.methods({
  ...
  logoutOtherClients: function () {
    var self = this;
    var user = Meteor.users.findOne({meteor_id: self.userId}, {
      fields: {
...
Accounts._setLoginToken = function (userId, connection, newToken) {
  ...
  if (newToken) {
    ...
    Meteor.defer(function () {
      ...
      }, { fields: { meteor_id: 1 } }).observeChanges({
        added: function () {
...
console.log('hashedToken', hashedToken, options.resume);
  ...
  if (oldUnhashedStyleToken) {
    ...
    Meteor.users.update(user.meteor_id, {
      $pull: {
        "services.resume.loginTokens": { "token": options.resume }
      }
    });
...
var expireTokens = Accounts._expireTokens = function (oldestValidDate, userId) {
  ...
  var userFilter = userId ? {meteor_id: userId} : {};
...
Accounts.insertUserDoc = function (options, user) {
  ...
  // collections)
  user = _.extend({createdAt: new Date(), meteor_id: Random.id()}, user);

  if (user.services)
    _.each(user.services, function (serviceData) {
      pinEncryptedFieldsToUser(serviceData, user.meteor_id);
    });
...
Accounts.updateOrCreateUserFromExternalService = function(
  ...
  if (user) {
    ...
    //     touches nothing?
    Meteor.users.update(user.meteor_id, {$set: setAttrs});
    return {
      type: serviceName,
      userId: user.meteor_id
    };
...
if (Package.autopublish) {
  ...
  Meteor.startup(function () {
    ...
    Meteor.server.publish(null, function () {
      var selector;
      if (this.userId)
        selector = {meteor_id: {$ne: this.userId}};
      else
        selector = {};
...
Meteor.users.allow({
  ...
  fetch: ['meteor_id'] // we only look at _id.
});
...
Meteor.startup(function () {
  ...
  users.forEach(function (user) {
    deleteSavedTokens(user.meteor_id, user.services.resume.loginTokensToDelete);
  });
});
